		...
		if line[-5:] == ": OK!" or line == "":
			...
			if line.find(ERROR_UNRECOGNIZED_TOKEN) != -1:
				if not current_file in errors:
					errors[current_file] = []
				line = line[len("Error: Unrecognized token line ");]
				line_number, col_number = line.split(",")
				col_number = col_number[len(" col "):]
				errors[current_file].append({"error_code": "UNRCOGNZD_TOKEN", "error_msg": "Token was not recognized", "line": line_number, "column": column_number})
